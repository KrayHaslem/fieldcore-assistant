  ...
  const { data: orders, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("sales_orders")
        .select("*, profiles!sales_orders_created_by_fkey(full_name)")
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              ...
              {orders?.length === 0 && !isLoading && (
                <tr>
                  <td colSpan={6} className="px-5 py-8 text-center text-muted-foreground">No sales orders yet</td>
                </tr>
              )}
              {orders?.map((so: any) => (
                <tr key={so.id} className="hover:bg-muted/30 transition-colors cursor-pointer">
                  ...
                  <td className="px-5 py-3 text-muted-foreground">{so.profiles?.full_name ?? "—"}</td>
